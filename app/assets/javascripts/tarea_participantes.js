...
var boxSizeArray = [100,100,100,100,100,100,100];	// Array indicating how many items there is rooom for in the right column ULs
...
function initDrag(e)	// Mouse button is pressed down on a LI
{
   ...
   //No deja que se seleccionen las tareas de la columna aprobadas
    if(padre.id=='box3')return;

    var prioridad;
    if(padre.id=='allItems')
    {
        if (validar_prioridad(this.getAttribute('proceso'),this.getAttribute('prioridad')))
            return;
    }

    var st = -100;//Math.max(document.body.scrollTop,document.documentElement.scrollTop);
    var sl = -50;// Math.max(document.body.scrollLeft,document.documentElement.scrollLeft);
...
function dragDropEnd(e)
{
    ...
    if(destinationObj)
    {
        ...
        tarea= contentToBeDragged.getAttribute('id').substr(4);

        if(destinationObj.id=='allItems')
        {
            columna= 1;
            updateEstado();
        }
        else if (destinationObj.id=='box1')
        {
            columna=2;
            updateEstado();
        }
        else if (destinationObj.id=='box2')
        {
            columna=3;
            updateEstado();
        }
        else if (destinationObj.id=='box3')
        {
            //No deja pasar tareas a la columna 4
        }
        ...
        if(destinationObj.id=='allItems' || destinationObj.parentNode.id=='allItems')
            contentToBeDragged.className='';
